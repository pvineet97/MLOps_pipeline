...
from Lead_scoring_data_pipeline.mapping.significant_categorical_level import list_platform,list_medium,list_source
...
def map_city_tier():
    ...
    db_path=os.path.join(DB_PATH,DB_FILE_NAME)
    conn = sqlite3.connect(db_path)
    df = pd.read_sql_query(f'''select * from loaded_data ; ''',conn)
...
def interactions_mapping():
    ...
    df_pivot.to_sql('interactions_mapped',conn,index=False,if_exists='replace')
    df_model_input = df_pivot.drop(NOT_FEATURES, axis=1)
    df_model_input.to_sql('model_input',conn,index=False,if_exists='replace')
    conn.close()  